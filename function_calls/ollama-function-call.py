...
from function_calls.config import Config
from function_calls.tools.toolbox import TOOL_NAMES_TO_FUNCTIONS
...
def explain_tool_calls_in_model_response(model_response, call_function=False):
    ...
    if call_function is True:
        print(
            f"The result of the function is: {TOOL_NAMES_TO_FUNCTIONS[tool_name](**function_args)}"
        )
    print("")


# invoke the model with a question, hopefully it will use the right tool
explain_tool_calls_in_model_response(
    model.invoke("What is the current weather in New York City?"), True
)
explain_tool_calls_in_model_response(
    model.invoke("What is the current pollen level in New York City?"), True
)
